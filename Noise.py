...
def sin_cos(x, z):
    return np.sin(x) * 2 + np.sin(z) * 2 + noise.noise2(x, z) * 11 - rd.random() * 10

x = np.linspace(-200, 200, 400)
y = np.linspace(-200, 200, 400)

#z = np.array([[noise.noise2(i, j) for i in x] for j in y])
z = np.array([[sin_cos(i, j) for i in x] for j in y])


plt.figure(figsize=(8, 6))
plt.contourf(x, y, z)
plt.colorbar(label='Noise Value')
plt.title('Simplex Noise (2D)')
plt.ylabel('Y')
plt.show()
